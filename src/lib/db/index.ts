...
export async function readRecord(event_fetch: (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>): Promise<Array<RecordDB>> {
...
export function retrieveByCemetery(database: Array<RecordDB>, name: string): Array<RecordDB | undefined> {

    const fromCemetery = database.map((value) => {
...
export function retrieveByAge(database: Array<RecordDB>, age: number): Array<RecordDB | undefined> {
    ...
    const fromAge = database.map((value) => {
        if (value.age === age) {
            return value;
        }
    }).filter((value) => value !== undefined)
...
export function retrieveByCategory(database: Array<RecordDB>, category: string): Array<RecordDB | undefined> {
    const fromCategory = database.map((value) => {
...
export function retrieveByORNumber(database: Array<RecordDB>, ORNumber: number): Array<RecordDB | undefined> {

    const fromORNumber = database.map((value) => {
